...
def get_lastChangedate():
    engine = connection.get_engine()
    query = 'SELECT MAX("date") AS max_date FROM advertisement'
...
def fetch_data():
    load_dotenv('.env')
    # env_path = Path("/root/reklama/.env")
    ...
    for idx, chunk in enumerate(chunked(ids, 50)):
        ...
        while attempt < max_retries:
            ...
            if response.status_code in (429, 503):
                attempt += 1
                print(f"Chunk {idx+1}: Received {response.status_code} status, retrying attempt {attempt}/{max_retries} after {retry_wait_time} seconds...")
        ...
        if response.status_code != 200:
            print(f"Chunk {idx+1}: Failed with status code {response.status_code} after {attempt+1} attempts")
            continue
    ...
    final_df.to_excel("28apr-4may", index=False)
